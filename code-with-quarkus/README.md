...
기존에는 로그인 후 메인화면인 `/after_login`에서만 오늘의 랜덤 챔피언 기능을 확인할 수 있었다. 그러나 오늘의 랜덤 챔피언 기능은 로그인 여부와 관계없이 모든 사용자가 이용할 수 있는 정보성 기능이므로, 로그인 전 기본 메인화면인 `/index.html`에서도 확인할 수 있도록 수정하였다.
...
이 기능은 기존 REST API를 재사용하여 로그인 전 메인화면과 로그인 후 메인화면에서 모두 사용할 수 있도록 확장한 기능이다. 이를 통해 코드 재사용성과 웹사이트 전체 완성도를 높였다.
...
#### 1. 로그인 전 메인화면 공용 추천 기능

![로그인 전 메인화면 공용 추천 기능](screenshots/추가기능_로그인전_메인화면_공용추천기능.png)

#### 2. 로그인 전 랜덤 챔피언 추천 결과

![로그인 전 랜덤 챔피언 추천 결과](screenshots/추가기능_로그인전_랜덤챔피언_라인추천결과.png)
...
기존에는 로그인 후 메인화면인 `/after_login`에서만 라인별 빠른 추천 기능이 보였다. 하지만 라인별 챔피언 추천은 로그인하지 않은 사용자도 사용할 수 있는 공용 기능이므로, 로그인 전 기본 메인화면인 `/index.html`에도 동일하게 추가하였다.
...
#### 1. 로그인 전 메인화면 라인별 추천 영역

![로그인 전 메인화면 공용 추천 기능](screenshots/추가기능_로그인전_메인화면_공용추천기능.png)

#### 2. 로그인 전 라인별 추천 결과

![로그인 전 라인별 추천 결과](screenshots/추가기능_로그인전_랜덤챔피언_라인추천결과.png)
...
기존에는 로그인 후 메인화면에서만 챔피언 데이터 통계를 확인할 수 있었지만, 챔피언 데이터 통계는 로그인 여부와 관계없이 모든 사용자가 볼 수 있는 정보성 기능이다. 따라서 로그인 전 기본 메인화면인 `/index.html`에도 `📊 챔피언 데이터 통계` 영역을 추가하였다.

`search.js`에서는 `/champions` API로 DB에 저장된 챔피언 목록을 불러오고, JavaScript 배열을 기준으로 전체 챔피언 수, 라인별 챔피언 수, 난이도별 챔피언 수를 계산한다. 계산된 결과는 `mainChampionStatsResult` 영역에 카드 형태로 출력되며, 사용자는 메인화면에서 현재 등록된 챔피언 데이터 현황을 바로 확인할 수 있다.

이 기능을 통해 MySQL 데이터베이스에 저장된 챔피언 정보를 단순 검색에만 사용하는 것이 아니라, 통계 형태로 분석하여 화면에 보여주는 기능으로 확장하였다.
...
| `search.js`             | `/champions` API 데이터 기반 통계 계산 및 화면 출력 |
| `ChampionResource.java` | DB에 저장된 챔피언 목록을 JSON으로 제공하는 API     |
...
![로그인 전 챔피언 데이터 통계](screenshots/추가기능_로그인전_메인화면_공용추천기능.png)

#### 2. 로그인 전 통계 및 추천 결과 확인

![로그인 전 통계 및 추천 결과](screenshots/추가기능_로그인전_랜덤챔피언_라인추천결과.png)
...
| 메인화면 라인별 빠른 추천   | 로그인 전/후 메인화면에서 라인 버튼으로 챔피언 추천        |
| 메인화면 챔피언 데이터 통계 | 로그인 전/후 메인화면에서 전체, 라인별, 난이도별 통계 표시 |
| 데이터 유지 개선            | 서버 재시작 후에도 사용자 데이터가 유지되도록 수정         |

> 기존 수업에서 구현한 로그인, 세션, DB, REST API, 프로필 기능을 활용하여 사용자별 데이터를 저장하고 조회하는 맞춤형 웹 서비스 기능으로 확장하였다. 또한 챔피언 개인 메모 기능을 추가하여 사용자가 직접 데이터를 입력, 저장, 조회, 삭제할 수 있는 CRUD 기능을 구현하였고, 활동 통계와 여러 추천 기능을 통해 사용자 경험을 높였다. 추가로 로그인 전 기본 메인화면에도 챔피언 데이터 통계, 오늘의 랜덤 챔피언, 라인별 빠른 추천 기능을 적용하여 로그인하지 않은 사용자도 주요 정보를 확인할 수 있도록 개선하였다.
